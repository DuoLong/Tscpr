...
[pCur11,pGt,pGtN,pStar,imgIds,N,N1]=shapeGt('initTr',...
    IsTr,phisTr,model,[],bboxesTr,L,10);      %暂时不改---->我把此处的pCur改成了pCur11
load 'pCur.mat'  %完成pCur的替换
%initData=struct('pCur',pCur,'pGt',pGt,'pGtN',pGtN,'pStar',pStar,...
 %   'imgIds',imgIds,'N',N,'N1',N1);       %
initData=struct('pCur',pCur(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]),'pGt',pGt(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]),'pGtN',pGtN(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]),'pStar',pStar(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]),...
    'imgIds',imgIds,'N',N,'N1',N1);
%Create training structure
%下面一行新添加
model.D=27;   %自己加
model.nfids=9;   %自己加
regPrm.model.D=27;% 自己加
regPrm.model.nfids=9; %自己加
...
trPrm.regPrm.model.D=27;   %自加，前面已改可有可无
trPrm.regPrm.model.nfids=9;  %自加，前面已改可有可无
[regModel,~] = rcprTrain(IsTr,phisTr(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]),trPrm);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TEST
%Initialize randomly using RT1 shapes drawn from training
%p1=shapeGt('initTest',IsT,bboxesT,model,pStar(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]),pGtN(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]),RT1);
load 'pRT.mat'
p=pRT(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76],:);%改增
%Create test struct
regPrm.model.D=27;% 自己加，上面已改。
regPrm.model.nfids=9; %自己加，上面已改。
...
%Compute loss
loss = shapeGt('dist',regModel.model,p,phisT(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]));
fprintf('--------------DONE\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tzy=phisT(:,[2 4 7 8 10 12 15 16 18 31 33 36 37 39 41 44 45 47 60 62 65 66 68 70 73 74 76]);
